### Hey 👋

I'm Logan, an engineer from the north of England, currently working as an engineer on GenAI at @[CitiBank](https://www.citi.com/) via @[JUXT](https://www.juxt.pro/). Previously working on DBaaS platforms @[InfluxData](https://www.influxdata.com/).

- Find me on [linkedin](https://www.linkedin.com/in/logan-cox-251303163/) or my [website](https://logan-cox.com) 
- Reach out to me at mail@logan-cox.com 
- A strong interest in systems programming and distributed systems
- I enjoy building with [Rust](https://www.rust-lang.org/), [Go](https://go.dev/), [Kubernetes](https://kubernetes.io/) and [Terraform](https://www.terraform.io/) 
- Currently building
    - [quickstash](https://github.com/quickstash) a simple platform to store and retrieve JSON
    - [mage](https://github.com/logan-bobo/mage) a selfhostable platform for application deployment
    - [orbc-go](https://github.com/logan-bobo/obrc-go) 1 Billion Row Challenge in Go. Calculateing the min, max, and average of 1 billion measurements.
    - [prism-lb](https://github.com/logan-bobo/prism-lb) a layer 7 load balancer 
    - [spark-kv](https://github.com/logan-bobo/spark-kv) a simple key-value store 

